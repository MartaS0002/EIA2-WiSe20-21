...
// Die Funktion für das Gegner-Zug funktioniert leider nicht... 



interface UNOKARTEN {   // Interface den UNO-Karten
    img: string;
    kartenFARBE: string;
    katenNUMMER: number;
}

let userARRAY: UNOKARTEN [] = [];      // Array der Userkarten
let computerARRAY: UNOKARTEN [] = [];  // Array der Gegnerkarten
let ziehenARRAY: UNOKARTEN [] = [];   // Array für den Ziehen-Stapel
let ablegenARRAY: UNOKARTEN [] = [];  // Array für den Ablegen-Stapel



// Audios für Gewonnen/ Verloren/ keine Karten (Audios sind aus dem 1 Semester; EIA_1-Endaufgabe)
var looseWin: HTMLAudioElement[] = [
...
function generateNeuesDeck (): void {
...
*/
    ...
    let red1: UNOKARTEN = {      // rote Karten werden generiert
        katenNUMMER : 1,
        kartenFARBE : "red",
        img : "bilder/red1.png"
    };
    ...
    let yellow1: UNOKARTEN = {     // gelbe Karten werden generiert
        katenNUMMER : 1,
        kartenFARBE : "yellow",
    ...
    let blue1: UNOKARTEN = {      // blaue Karten werden generiert
        katenNUMMER : 1,
        kartenFARBE : "blue",
    ...
    let green1: UNOKARTEN = {     // grüne Karten werden generiert
        katenNUMMER : 1,
        kartenFARBE : "green",
...
function generateKartenHTML(): void { 

    // Userkarten werden generiert
    document.getElementById ("meineKarten").innerHTML = ""; // damit jew. nur eine (1) Karte generiert wird
    for (let i: number = 0; i <= userARRAY.length - 1 ; i++) {
        let userKarte: HTMLImageElement = document.createElement("img");
        userKarte.innerHTML = "";
        userKarte.setAttribute ("src", userARRAY[i].img);
        userKarte.addEventListener ("click", function (): void {karteSpielen (i); }, false);
...
function karteZiehen(): void {
    ...
    userARRAY.push(ziehenARRAY[0]);
    generateKartenHTML();
    setTimeout(function (): void { document.getElementById("img").add("animation"); }); // dies sollte Timeout/ Animation für den Gegner-Zug sein...
}
...
function karteSpielen (richtigeKarteKlick: number): void {
