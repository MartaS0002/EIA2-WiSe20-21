namespace L06_Hexenkessel_Server {
  ...
  async function handleLoad(_event: Event): Promise<void> {
    ...
    let send: HTMLButtonElement = <HTMLButtonElement>(
      document.querySelector("#btnSend")
    );
    send.addEventListener("click", sendRecipe);

    let btnGeneral: HTMLButtonElement = <HTMLButtonElement>(
      document.querySelector("#btnGeneral")
    );
    let btnIngredients: HTMLButtonElement = <HTMLButtonElement>(
      document.querySelector("#btnIngredients")
    );
    let btnTemperature: HTMLButtonElement = <HTMLButtonElement>(
      document.querySelector("#btnTemperature")
    );
    let btnStir: HTMLButtonElement = <HTMLButtonElement>(
      document.querySelector("#btnStir")
    );
    let btnDelete: HTMLButtonElement = <HTMLButtonElement>(
      document.querySelector("#delete")
    );

    let slider: HTMLInputElement = <HTMLInputElement>(
      document.querySelector("input#duration")
    );
    let slider2: HTMLInputElement = <HTMLInputElement>(
      document.querySelector("input#intensity")
    );
  ...
  function displayIntensity(_event: Event): void {
    let progress: HTMLProgressElement = <HTMLProgressElement>(
      document.querySelector("#intensitySlider")
    );
  ...
  async function sendRecipe(): Promise<void> {
    let querySendGeneral: URLSearchParams = new URLSearchParams(
      <any>formDataSendGeneral
    );
    let querySendInstructions: URLSearchParams = new URLSearchParams(
      <any>formDataSendInstructions
    );
    let responseGeneral: Response = await fetch(
      url + "?" + querySendGeneral.toString()
    );
    let responseInstructions: Response = await fetch(
      url + "?" + querySendInstructions.toString()
    );
  ...
  function askBeforeDelete(): void {
    let outputGeneral: HTMLDivElement = <HTMLDivElement>(
      document.querySelector("div#outputGeneral")
    );
    let outputInstructions: HTMLDivElement = <HTMLDivElement>(
      document.querySelector("div#outputInstructions")
    );
    if (
      outputGeneral.innerHTML &&
      outputInstructions.innerHTML != "" &&
      confirm("Hoffnung aufgegeben, Potter? (- ja...)")
    ) {
      deleteAll();
    }
  }
  ...
  function deleteAll(): void {
    ...
    let totalSpan: HTMLSpanElement = <HTMLSpanElement>(
      document.querySelector("#totalSpan")
    );
    outputGeneral.innerHTML = "...";
    outputInstructions.innerHTML = "";
    totalSpan.innerHTML = "";
  ...
  function displayGeneral(): void {
    ...
    let formDataGeneral: FormData = new FormData(
      <HTMLFormElement>document.querySelector("#formGeneral")
    );
    ...
    for (let entry of formDataGeneral) {
      switch (entry[0]) {
        ...
        case "Wirkungsdauer (Minuten)":
          if (entry[1] != "0" && effect) {
            outputGeneral.innerHTML +=
              "Wirkungsdauer: " + entry[1] + " Minute(n)" + "<br>";
            formDataSendGeneral.delete(entry[0]);
            formDataSendGeneral.append(entry[0], entry[1]);
          }
          break;
  ...
  function displayIngredients(): void {
    ...
    let formDataInstructions: FormData = new FormData(
      <HTMLFormElement>document.querySelector("#formInstructions")
    );
    ...
    for (let entry of formDataInstructions) {
      switch (entry[0]) {
        case "Rühren mit Intensität":
          if (entry[1] != "0") {
            outputInstructions.innerHTML +=
              "Rühren mit einer Intensität von " + entry[1] + "/10." + "<br>";
            intensity = true;
            formDataSendInstructions.append(x + ". " + entry[0], entry[1]);
            x++;
          }
  ...
  function convertCurrency(_total: number): string {
    ...
    if (galleonen != "0") {
      adjustedPrice =
        galleonen + " Galleonen, " + sickel + " Sickel und " + knut + " Knut";
    } else if (sickel != "0") {
