namespace L08_Skipiste {
  ...
  function handleload(_event: Event): void {
    ...
    drawBackground();
    drawSun({ x: 50, y: 25 });
    drawCloud({ x: 200, y: 105 }, { x: 250, y: 75 });
    drawPiste();
    drawKurve();
    drawLift();
    drawTrees();
  ...
  function drawSkier(): void {
    let skiCount: number = 7;

    crc2.save();
    for (let i: number = 0; i < skiCount; i++) {
      let x: number = Math.random() * 500 + 100;
      let y: number = Math.random() * 850 + 200;
  ...
  function drawSingleSkifahrer(_position: Vector): void {
    ...
    let skincolor: string = "HSL(30, 100%, " + Math.random() * 100 + "%)";
